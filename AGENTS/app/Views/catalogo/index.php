...
<section class="catalog-container">
  ...
  <div class="products-grid static-grid">
    <?php foreach(array_slice($ofertas,0,6) as $p): ?>
      <article class="product-card"><img src="<?= e($p['portada'] ?: asset_url('img/logo.png')) ?>" alt=""><div class="product-info"><span class="badge-sale"><?= e($p['oferta_descuento'] ?? '') ?>% OFF</span><h3><?= e($p['titulo'] ?? 'Producto') ?></h3><small>Antes <?= money_ar($p['precio_base'] ?? $p['precio'] ?? 0) ?></small><strong><?= money_ar($p['precio_final'] ?? $p['precio_venta'] ?? 0) ?></strong></div></article>
    <?php endforeach; ?>
  </div>
  <?php endif; ?>
