...
// Edición completa: título, precio, descripción, categorías, talles y fotos.
...
const fVisibleEstado = $("fVisibleEstado");
const fDisponibilidad = $("fDisponibilidad");
const fConFoto = $("fConFoto");
const fOrden = $("fOrden");
...
function buildCategoryFilterDisplay(){
  const usedIds = new Set(cache.flatMap((p) => p.categorias_ids || []).map((id) => String(id)));
  const groups = new Map();
  categoriasAll
    .filter((c) => usedIds.size === 0 || usedIds.has(String(c.id)))
    .forEach((c) => {
  ...
  categoriasDisplay = Array.from(groups.values())
    .sort((a, b) => a.nombre.localeCompare(b.nombre, "es", { sensitivity: "base" }));
  categoryFilterAliases = new Map(categoriasDisplay.map((c) => [String(c.id), c.aliasIds.slice()]));
}

function renderCategoriaFilter(){
  if (!fCategoria) return;
  const actual = String(fCategoria.value || "");
  const opciones = categoriasDisplay
    .slice()
    .sort((a, b) => String(a.nombre || "").localeCompare(String(b.nombre || ""), "es", { sensitivity: "base" }))
    .map(c => `<option value="${c.id}">${esc(c.nombre || `Categoría #${c.id}`)}</option>`)
    .join("");

  fCategoria.innerHTML = `<option value="">Todas las categorías</option>${opciones}`;
...
function render(){
  ...
  const categoriaId = String(fCategoria?.value || "");
  const visibleEstado = String(fVisibleEstado?.value || "");
  const disponibilidad = String(fDisponibilidad?.value || "");
  const conFoto = String(fConFoto?.value || "");
  const orden = String(fOrden?.value || "recientes");

  let rows = cache.slice();

  if (visibleEstado === "visible") rows = rows.filter(x => !!x.visible);
  if (visibleEstado === "oculto") rows = rows.filter(x => !x.visible);
  if (disponibilidad === "disponible") rows = rows.filter(x => !!x.disponible);
  if (disponibilidad === "nodisponible") rows = rows.filter(x => !x.disponible);
  if (conFoto === "confoto") rows = rows.filter(x => !!x.portada_url);
  if (conFoto === "sinfoto") rows = rows.filter(x => !x.portada_url);
  if (categoriaId) {
    const aliasIds = categoryFilterAliases.get(categoriaId) || [categoriaId];
    rows = rows.filter(x => (x.categorias_ids || []).some(id => aliasIds.includes(String(id))));
  }
  ...
  if (term){
    rows = rows.filter(x => {
      ...
      const precio = String(x.precio ?? "").toLowerCase();
      return t.includes(term) || c.includes(term) || id.includes(term) || precio.includes(term);
    });
  }

  rows.sort((a, b) => {
    if (orden === "antiguos") return Number(a.id || 0) - Number(b.id || 0);
    if (orden === "titulo_asc") return String(a.titulo || "").localeCompare(String(b.titulo || ""), "es", { sensitivity: "base" });
    if (orden === "titulo_desc") return String(b.titulo || "").localeCompare(String(a.titulo || ""), "es", { sensitivity: "base" });
  ...
  const body = rows.map(p => {
    const cats = (p.categorias || []).slice(0,3).map(n => `<span class="chip chipCategory">${esc(n)}</span>`).join("");
    const moreCats = (p.categorias || []).length > 3 ? `<span class="chip chipCategory">+${(p.categorias.length-3)}</span>` : "";

    const ts = (p.talles || []).slice(0,5).map(n => `<span class="chip chipTalle">${esc(n)}</span>`).join("");
    const moreT = (p.talles || []).length > 5 ? `<span class="chip chipTalle">+${(p.talles.length-5)}</span>` : "";

    const estado = `
      <div class="chips chipsStatus">
        <span class="chip chipState ${p.visible ? "isVisible" : "isHidden"}">${p.visible ? "Visible" : "Oculto"}</span>
        <span class="chip chipState ${p.disponible ? "isAvailable" : "isUnavailable"}">${p.disponible ? "Disponible" : "No disponible"}</span>
      </div>
    `;

    return `
      <tr>
        <td>${p.portada_url ? `<img class="thumb thumbProducto" src="${esc(p.portada_url)}" alt="${esc(p.titulo || 'Producto')}">` : `<div class="thumb thumbProducto thumbPlaceholder">Sin foto</div>`}</td>
        <td>${p.id}</td>
        <td>
          <div><strong>${esc(p.titulo || "-")}</strong></div>
          <div class="muted2">#${p.id}</div>
        </td>
        <td class="hideMobile"><div class="chips">${cats || `<span class="chip chipCategory">Sin categoría</span>`}${moreCats}</div></td>
        <td class="hideMobile"><div class="chips">${ts || `<span class="chip chipTalle">Sin talle</span>`}${moreT}</div></td>
        <td><strong>${money(p.precio)}</strong></td>
        <td>${estado}</td>
        <td>
          <div class="actionsRow actionsProducts">
...
async function toggleCampo(id, campo, valor){
  ...
  const { data, error } = await supabase
    ...
    .update(payload)
    .eq("id", id)
    .select("id,visible,disponible")
    .maybeSingle();

  if (error){
    console.error(error);
    showToast("No se pudo actualizar el producto.", "error");
    return showStatus(`No se pudo actualizar: ${error.message || "mirá consola."}`, true);
  }

  const p = cache.find(x => Number(x.id) === Number(id));
  if (p) {
    p[campo] = data && Object.prototype.hasOwnProperty.call(data, campo) ? data[campo] : !!valor;
    if (data?.visible !== undefined) p.visible = data.visible;
    if (data?.disponible !== undefined) p.disponible = data.disponible;
  }

  render();
  showToast(campo === "visible" ? (valor ? "Producto visible en catálogo." : "Producto oculto del catálogo.") : "Producto actualizado.", "success");
}
...
async function deleteFotoExistente(fotoId){
  ...
  if (error){
    ...
    return showMStatus(`No se pudo eliminar la foto: ${error.message || "mirá consola."}`, true);
  }

  // Si esa imagen también estaba siendo usada como portada, limpiamos productos.portada_url.
  // Esto es clave para que desaparezca del catálogo público, no solo del admin.
  if (foto?.url){
    const { data: prodActual, error: prodReadError } = await supabase
      .from("productos")
      .select("id,portada_url")
      .eq("id", editId)
      .maybeSingle();

    if (prodReadError) console.warn("No se pudo leer portada actual:", prodReadError);

    const portadaEnBase = prodActual?.portada_url || portadaActualUrl || "";
    if (String(portadaEnBase) === String(foto.url)){
  ...
  const prod = cache.find(x => Number(x.id) === Number(editId));
  if (prod) {
    prod.fotos = (prod.fotos || []).filter(f => Number(f.id) !== Number(fotoId));
    if (foto?.url && String(prod.portada_url || "") === String(foto.url)) prod.portada_url = "";
  }

  renderFotosExistentes();
  render();
  showMStatus("Foto eliminada. También se actualizó la vista del catálogo.");
...
async function saveEdit(){
  ...
  if (!titulo) return showMStatus("El título es obligatorio.", true);
  if (precio === null || precio < 0) return showMStatus("Precio inválido.", true);
...
fVisibleEstado?.addEventListener("change", render);
fDisponibilidad?.addEventListener("change", render);
fConFoto?.addEventListener("change", render);
fOrden?.addEventListener("change", render);
btnLimpiarFiltros?.addEventListener("click", () => {
  q.value = "";
  if (fCategoria) fCategoria.value = "";
  if (fVisibleEstado) fVisibleEstado.value = "";
  if (fDisponibilidad) fDisponibilidad.value = "";
  if (fConFoto) fConFoto.value = "";
  if (fOrden) fOrden.value = "recientes";
  render();
});
