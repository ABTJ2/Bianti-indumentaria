...
function confirmDanger(label, period){
  console.warn(`Borrado legacy cancelado para ${label} (${period.from} a ${period.to}).`);
  return false;
}
...
async function refreshCounts(){
  ...
  try {
    ...
    console.warn("Error actualizando conteos. Revisá consola.");
  }
}
...
btnCsvContabilidad.addEventListener("click", async () => {
  const period = getPeriod(); if (!period) { console.warn("Falta período."); return; }
  try { await exportContabilidad(period); } catch (e) { console.error(e); console.warn("Error exportando contabilidad."); }
});
btnCsvVentas.addEventListener("click", async () => {
  const period = getPeriod(); if (!period) { console.warn("Falta período."); return; }
  try { await exportVentas(period); } catch (e) { console.error(e); console.warn("Error exportando ventas."); }
});
btnCsvManuales.addEventListener("click", async () => {
  const period = getPeriod(); if (!period) { console.warn("Falta período."); return; }
  try { await exportManuales(period); } catch (e) { console.error(e); console.warn("Error exportando manuales."); }
});
btnCsvEventos.addEventListener("click", async () => {
  const period = getPeriod(); if (!period) { console.warn("Falta período."); return; }
  try { await exportEventos(period); } catch (e) { console.error(e); console.warn("Error exportando eventos."); }
});

btnDelVentas.addEventListener("click", async () => {
  const period = getPeriod(); if (!period) { console.warn("Falta período."); return; }
  try { await delVentas(period); await refreshCounts(); } catch (e) { console.error(e); console.warn("Error borrando ventas. Puede ser RLS/policies."); }
});
btnDelManuales.addEventListener("click", async () => {
  const period = getPeriod(); if (!period) { console.warn("Falta período."); return; }
  try { await delManuales(period); await refreshCounts(); } catch (e) { console.error(e); console.warn("Error borrando manuales. Puede ser RLS/policies."); }
});
btnDelEventos.addEventListener("click", async () => {
  const period = getPeriod(); if (!period) { console.warn("Falta período."); return; }
  try { await delEventos(period); await refreshCounts(); } catch (e) { console.error(e); console.warn("Error borrando eventos. Puede ser RLS/policies."); }
});
