...
async function cargar() {
  ...
  if (error) {
    console.error(error);
    console.warn("Error cargando categorías");
    return;
  }
...
async function upsertCategoria({ id = null, nombre, orden, visible }) {
  ...
  if (resp.error) {
    ...
    console.warn("No se pudo guardar la categoría");
    return;
  }
...
btnNueva?.addEventListener("click", async () => {
  console.warn("Alta legacy cancelada. Usá el formulario MVC de categorías.");
});
...
tabla?.addEventListener("click", async (e) => {
  ...
  if (action === "edit") {
    ...
    if (error) {
      console.error(error);
      console.warn("No se pudo cargar la categoría para editar");
      return;
    }

    console.warn("Edición legacy cancelada. Usá el formulario MVC de categorías.");
  }
});
