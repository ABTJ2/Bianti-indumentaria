...
async function crearCategoria(){
  ...
  // por defecto: usa talles = true (después lo podés editar en Categorías)
  const payload = { nombre: nombre.trim(), orden: 999, visible: true, usa_talles: true };
...
async function guardar(){
  ...
  if(!t) return setStatus("Falta el nombre del producto.");
  if(pv == null || pv < 0) return setStatus("Precio de venta inválido.");
  if(pc != null && pc < 0) return setStatus("Precio de costo inválido.");
  if(!portada) return setStatus("Falta la portada (archivo).");
  ...
  try{
    ...
    if(extras.length){
      ...
      for(let i=0;i<extras.length;i++){
        const url = await uploadToBucket(extras[i], "extras");
        fotos.push({ producto_id: productoId, url, orden: i+1 });
      }
    }
    ...
    setStatus("✅ Producto guardado con éxito. Formulario listo para el siguiente.");
    resetForm();
  }catch(err){
    console.error(err);
    setStatus(`Error: ${err.message || "No se pudo guardar."}`);
  }finally{
...
btnNuevaCategoria.addEventListener("click", crearCategoria);
