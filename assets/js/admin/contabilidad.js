...
function drawChart(series){
  ...
  chart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        { label: "Ingresos", data: series.ingresos, tension: 0.25 },
        { label: "Recuperado (costo)", data: series.recuperado, tension: 0.25 },
        { label: "Ganancia (referencia)", data: series.ganancia, tension: 0.25 },
      ]
    },
    options: {
      responsive: true,
      ...
      scales: {
        y: { ticks: { callback: (v)=> money(v).replace("ARS","").trim() } }
      }
    }
  });
