...
mVender.addEventListener("click", async ()=>{
  try{
    ...
    closeModalPedido();
    await load();
    alert("Vendido y registrado en ventas (impacta en contabilidad).");
  } catch (e){
    console.error(e);
    alert(e.message || "Error vendiendo");
  }
});
...
vmGuardar.addEventListener("click", async ()=>{
  try{
    ...
    closeModalManual();
    alert("Venta manual guardada (impacta en contabilidad).");
  } catch (e){
    console.error(e);
    alert(e.message || "Error guardando venta manual");
  }
});
